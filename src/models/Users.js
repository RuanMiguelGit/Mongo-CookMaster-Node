...
.then((db) => db.collection('recipes').insertOne({name, ingredients, preparation, userId:data}))
.then((result) => result)
return datas.ops[0]
}
...
const data = await connection()
.then((db) => db.collection('recipes').findOneAndUpdate({ _id:  ObjectId(id) }, {$set:{name, ingredients, preparation}},
...
const data = await connection()
.then((db) => db.collection('recipes').findOneAndUpdate({ _id:  ObjectId(id) }, {$set:{image:url}},
 {returnOriginal:false}))
 .then((result) => result.value)
return data
}
